...
int grado_verticec3(Grafoc3 *g, int v){
	int cuenta=0;
	if (existe_verticec3(g,v)){ // existe vértice v?
		for (int j=0; j<g->n; j++) {
			if (g->Mad[v][j] != 0.0)
				cuenta = cuenta + 1;
		}
...
int *adyacentes_verticec3(Grafoc3 *g, int v){
	...
	if (grado_verticec3(g,v)){ // existe vértice v?
...
bool agregar_aristac3(Grafoc3 *g, Aristac3 a){
	if (existe_verticec3(g,a.v)){ // existe vértice v?
		if (existe_verticec3(g,a.w)){ // existe vértice w?
...
void mostrar_adyacentesc3(Grafoc3 *g, int v){
			//Se asume que antes de llamar a esta operación ha validado que adyacentes existe
			for (int j=0; j<g->n; j++) {
				if (g->Mad[v][j] != 0.0){
						printf("\n  %d", j);
			  }
